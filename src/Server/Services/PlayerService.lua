...
function PlayerService:_listenForDeaths(player)
	self._playerEvents[player.Name .. "_spawned"] = player.CharacterAdded:Connect(function(character)
		...
		self._playerEvents:Update(player.Name .. "_died", humanoid.Died:Connect(function()
...
function PlayerService:Start()
	...
	local onPlayerAdded = function(pl) self:_onPlayerAdded(pl) end
	...
	if self.Modules.Settings.UseTestPlayers then
		local testPlayer = self.Shared.TestPlayer.new(game.Players:WaitForChild("ModuleMaker"), 
				{ Character = workspace:WaitForChild("TestPlayer"), Name = "TestPlayer"})
